...
namespace PowerTalkBox
{
    ...
    DescriptionAttribute("过期次数:HttpContext.Current.Session过期的最大跳转次数")
        ...
        public   string OutLocation
        {
            get { 
                ...
                return (LocState == null) ? "http://www.commelp.org" : LocState.ToString();  
                }
   ...
   [CategoryAttribute("PowerTalk选项"),
    DescriptionAttribute("广告翻滚内容:URL地址")
        ...
        [CategoryAttribute("用户列表变化时调用的JS"),
       DescriptionAttribute("调用的JS:方法")
        ]
        public string UserChangeList
        {
            get
            {
                object LocState = this.ViewState["UserChangeList"];
                return (LocState == null) ? "ShowMsg" : LocState.ToString();
            }
            set { this.ViewState["UserChangeList"] = value.ToString(); }
        }
...
href=""#"" target=_blank>欢迎访问移动商城!</A></DIV></LI>
<LI style=""HEIGHT: 25px"">
<DIV class=slideFilm 
style=""OVERFLOW: hidden; WIDTH: 100%; HEIGHT: 25px; TEXT-ALIGN: left""><A 
href=""#"" target=_blank>我们将竭诚为您服务!</A></DIV></LI>
<LI style=""HEIGHT: 25px"">
<DIV class=slideFilm 
style=""OVERFLOW: hidden; WIDTH: 100%; HEIGHT: 25px; TEXT-ALIGN: left""><A 
href=""#"" 
target=_blank>客户至上使我们的宗旨!</A></DIV></LI>" : PowerTalkBoxContrls.Design.Html.WebClientGetSource(ADNetInfoString)) + @"
         ...
         string TableHalfBB = @" </div> 
              <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">
              <tr>
              <td class=""pt-appsend"" style=""height: 25px""><label><a href='#' target='_blank'><img id=""PowerTalkImg"" src=""images/AdMiddle.png"" /></a>
          </table>
...
function GetArea(areaID){ 
 areaId=areaID;
    ...
    ";
        ...
        /// </summary>
        /// <returns></returns>
        public string ListScriptChanged()
        {
            if (!string.IsNullOrEmpty(UserChangeList))
            {
                return UserChangeList + "(listContrl.innerHTML,(UserList.split('|'))[1]);";
            }
            else
            {
                return "";
        ...
        /// 提醒
        /// </summary>
        public string MsgBoxScript = @"function CLASS_MSN_MESSAGE(id,width,height,caption,title,message,target,action){  
    this.id     = id;  
    this.title  = title;  
...
CLASS_MSN_MESSAGE.prototype.oncommand = function(){  
    //this.close = true;
    this.hide();  
	window.open(""http://Powertalkbox.cnblogs.com"");
...
CLASS_MSN_MESSAGE.prototype.show = function(){  
    ...
    var str = ""<DIV style='BORDER-RIGHT: #455690 1px solid; BORDER-TOP: #a6b4cf 1px solid; Z-INDEX: 99999; LEFT: 0px; BORDER-LEFT: #a6b4cf 1px solid; WIDTH: "" + w + ""px; BORDER-BOTTOM: #455690 1px solid; POSITION: absolute; TOP: 0px; HEIGHT: "" + h + ""px; BACKGROUND-COLOR: #c9d3f3'>""  
        ...
        str += ""<DIV style='WORD-BREAK: break-all' align=left><A href='alert('有人上线')' hidefocus=false id='btCommand'><FONT color=#ff0000>"" + this.message + ""</FONT></A><A href='http://Powertalkbox.cnblogs.com' hidefocus=false id='ommand'><FONT color=#ff0000></FONT></A></DIV>""  
        str += ""</DIV>""  
        str += ""</TD>""  
        str += ""</TR>""  
        str += ""</TABLE>""  
...
{
var MSG1 = new CLASS_MSN_MESSAGE(""aa"",200,120,""消息提示："",""有人上线"",""妙思提醒您,咨询列表有变化!"");  
        ...
        public string GetCallbackResult()
        {
            ...
            switch (area[0].ToString())
            {
                case "0"://取信息
                    ...
                    //判断是否被踢
                    if (!PowerTalk.ExistUser(this.MyUserName))
                    {
                        if (OneLoginOut != null)
                        {
                            EventArgs ags = new EventArgs();
